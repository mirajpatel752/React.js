...
const Autocomplete = props => {
  ...
  const onKeyDown = e => {
    ...
    if (e.keyCode === 38 && activeSuggestion !== 0) {
      ...
      setShowSuggestions(false)
      setUserInput('')
    } else if (e.keyCode === 13 && showSuggestions && filteredData.length) {
      // ** User Pressed ENTER
      props.isGlobalSearch ? onSuggestionItemClick(filteredData[activeSuggestion].link, e) : props.onSuggestionItemClick(filteredData[activeSuggestion].link, filteredData[activeSuggestion][filterKey])
  ...
  const renderSuggestions = () => {
    ...
    if (grouped === undefined || grouped === null || !grouped) {
      ...
      suggestions.map(suggestion => {
        ...
        final_option_data.push({ ...suggestion, data: sortData })
        filteredData.push(...sortData)
      })
      return (!filteredData.length ? <li className='suggestion-item no-result'>
        <AlertCircle size={15} /> <span className='align-middle ms-50'>No Result</span>
      </li> : <>
        {final_option_data.map(option => <>
          {option.data.length ? <Fragment key={option[filterHeaderKey]}>
            <li className='suggestion-item suggestion-title-wrapper'>
              <h6 className='suggestion-title'>{option[filterHeaderKey]}</h6>
            </li>
            {renderGroupedSuggestion(option.data)}
          </Fragment> : ''}
        </>)}
      </>)
    }
  ...
  return (
    <div className={props.isGlobalSearch ? 'autocomplete-container' : ''} ref={container}>
      <Input
        type='text'
        onChange={e => {
          onChange(e)
      ...
      />
      {props.invalid && <FormFeedback tooltip={true}>
        {props.error_message} </FormFeedback>}
      {suggestionsListComponent}
    </div>
  )
}
