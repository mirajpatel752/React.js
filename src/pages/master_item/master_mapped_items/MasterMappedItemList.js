...
import { UncontrolledButtonDropdown, Button, DropdownItem, Card, DropdownToggle, DropdownMenu, Tooltip } from 'reactstrap'
...
import AddChannelSidebar from './AddChannel'
...
const ChannelItemList = ({ tabRender, setResetModuleData, resetModuleData, activeTabId, tabId, getCustomFilterList, fullScreenTitleShow,
  ...
  const [itemPublishToolTip, setItemPublishToolTip] = useState(false)
  const [itemPublishIndex, setItemPublishIndex] = useState('')
  const [itemPublishSubIndex, setItemPublishSubIndex] = useState('')
  ...
  const getChannelItemList = async (otherFilter, availableData, filter_id, activeTabId, search_activity = '') => {
    ...
    const res = await GetApiCall('GET', `${CommonApiEndPoint.get_master_mapped_items}?channel_type=0&per_page=50&limit=${otherFilter?.currentPage}&filter=${search_activity}&is_action=${activeTabId}&col_filter=${otherFilter?.columnFilters?.length ? encodeURIComponent(JSON.stringify(otherFilter.columnFilters)) : ''}&order_field=${otherFilter?.sorting?.length && otherFilter?.sorting[0]?.id}&order_by=${otherFilter?.sorting && otherFilter?.sorting[0]?.id ? otherFilter?.sorting[0]?.desc ? 'desc' : 'asc' : ''}&filter_data=${otherFilter?.is_filter_apply ? encodeURIComponent(JSON.stringify(otherFilter?.filter_value)) : ''}`, header)
  ...
  const accessRightsForAction = (data) => {
    return channelItemListData.length ? <div className='column-action munim-payment-list-btn'>
      <UncontrolledButtonDropdown>
        ...
        {isMobile ? <DropdownToggle color=''><MoreVertical size={15} /></DropdownToggle> : ""}
  ...
  const columns = [
    ...
    },
    {
      ...
      enableColumnActions: false,
      enableSorting: false,
      Cell: ({ row }) => <span className='munim-font-color d-flex'>{row.original.published_on && row.original.published_on.map((item, i) => {
        return (
          <div>
            <img key={i} id={`item_img_${row.index + 1}_${i}`} className="published-table-img" src={channel_logo[item.channel_type]} alt="Channel logo" />
            <Tooltip
              placement='top'
              isOpen={itemPublishToolTip && itemPublishIndex === (row.index + 1) && itemPublishSubIndex === i}
              target={`item_img_${row.index + 1}_${i}`}
              toggle={() => { setItemPublishToolTip(!itemPublishToolTip); setItemPublishIndex(row.index + 1); setItemPublishSubIndex(i) }}
            >
              {
                item.companies && item.companies.map((itemCom, i) => {
                  if (i < 3) {
                    return (
                      <span key={i}>
                        {itemCom.slice(3)}
  ...
  return (
    <Fragment>
      ...
      {addChannelSidebarOpen ? <AddChannelSidebar open={addChannelSidebarOpen} toggleSidebar={categorySidebar} addChannelData={addChannelData} /> : null}
    </Fragment >
  )
}
