...
import { channel_logo, detectBrowser } from '../../helper/commonFunction'
import { bucketPathUrl } from '../../helper/commonApi'
import { useSelector } from 'react-redux'

const MasterImportData = () => {
    ...
    const getImportFileLogListData = async (channel, flag = false) => {
        const header = { 'access-token': localStorage.getItem('access_token') }
        const res = await GetApiCall('GET', `${CommonApiEndPoint.get_import_progress}?id=${channel.id}`, header)
        if (res.data.status === 'success') {
            ...
            if (res.data.data.import_progress_per !== 100 && history.location.pathname.includes("master-import-data")) {
                setProgressShow(true)
                setTimeout(() => {
                    const channel = channelOption.find(ele => ele.value === selectedChannel)
                    getImportFileLogListData(channel)
                }, 2000)
            }
        } else {
            setProgressShow(false)
    ...
    async function fetchData() {
        const header = { 'access-token': localStorage.getItem('access_token') }
        const res = await GetApiCall('GET', `${CommonApiEndPoint.get_master_import_list}`, header)
        if (res.data.status === 'success') {
            const final_data = []
    ...
    useEffect(() => {
        if (channelOption.length) {
            const channel = channelOption.find(ele => ele.value === selectedChannel)
            getImportFileLogListData(channel, true)
        }
    }, [channelOption])
    /**
     * IW0110
     * This function is called when user download sample file data
     */
    const getChannelSampleFile = async (id, type, name) => {
        const header = { 'access-token': localStorage.getItem('access_token') }
        const res = await GetApiCall('GET', `${CommonApiEndPoint.get_import_sample_file}?id=${id}&channel_type=${type}&channel_name=${name}`, header)
        if (res.data.status === 'success') {
            if (browser_name === 'Edge') {
                location.href = `${bucketPathUrl}import_data/download_sample/${res.data.data}`
            } else {
                window.open(`${bucketPathUrl}import_data/download_sample/${res.data.data}`, '_self')
            }
            notify(res.data.message, 'success')
        } else {
    ...
    /**
     * IW0110
     * This effect is called when import data from channel list then update selected company channel
     */
    useEffect(() => {
        if (location_state?.is_channel_import && channelOption.length) {
            let active_value
            channelOption.forEach((ele, index) => {
                if (ele.text?.split('-')[1]?.trim() === selected_company_object.company_name) {
                    active_value = index
                }
            })
            setSelectedChannel(active_value)
        }
    }, [location_state?.is_channel_import, channelOption])
            ...
            <Row className='justify-content-center'>
                <Col lg='8' sm='12'>
                    <div className="munim-list-company">
                        <ModuleTitle breadCrumbTitle='Import Channel Item' />
                    </div>
                                ...
                                <CardBody>
                                    <Col md='6' >
                                        ...
                                        <FixSelect
                                            ...
                                            handleChange={(_, value) => setSelectedChannel(value)}
                                        />
                                    </Col>
                                    <div className='d-sm-flex d-block mt-2'>
                                        <div className='munim-import-icon mb-1 mb-sm-0 text-center'>
                                        ...
                                        <div className='mx-2 my-auto'>
                                            <h3>Import Item </h3>
                                            <p>Suppose you want import items from your desktop. <Button outline className='munim-button-link-none munim-display-contents' onClick={(e) => { e.preventDefault(); getChannelSampleFile(channelOption[selectedChannel]?.id, channelOption[selectedChannel]?.value, channelOption[selectedChannel]?.text) }}>Download from here</Button></p>
                                        </div>
                                    </div>
                                    <div className='d-flex mt-2'>
                                        <Button disabled={channelOption?.length > 0 ? progressShow : true} tag={Label} color='primary'>
...
export default MasterImportData